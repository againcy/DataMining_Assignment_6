...
namespace DataMining_Assignment_6
{
    ...
    class DecisionTree
    {
        ...
        /// <returns>信息熵</returns>
        public static double Entropy(DataSet curSet)
        {
            double result = 0;
            double[] cntLabel = new double[MaxLabel];
            cntLabel.Initialize();
            foreach (var d in curSet.Examples) cntLabel[d.label] += 1;
        ...
        public static double SplitEntropy_Discrete(DataSet curSet, int splitFeature, out double metric)
        {
            double result = 0;
            metric = 0;
            int N = curSet.Examples.Count;
        ...
        public static double[] PossibleThreshold(DataSet curSet, int splitFeature)
        {
            SortedSet<double> value = new SortedSet<double>();
            //double[] arr = new double[curSet.Examples.Count];
            foreach(var d in curSet.Examples)
            {
                var v = d.features[splitFeature];
                if (v != -1) value.Add(v);
            }
            ...
            //使用所有取值的25%,50%,75%处值作为划分阈值
            double[] result = new double[3];
            result[0] = arr[arr.Length / 4];
            result[1] = arr[arr.Length * 2 / 4];
            result[2] = arr[arr.Length * 3 / 4];
            return result;
        ...
        /// <param name="metric">当前划分的信息度量，用来归一化信息增益</param>
        /// <returns>划分的信息熵</returns>
        public static double SplitEntropy_Numeric(DataSet curSet, int splitFeature, out double bestThreshold, out double metric)
        {
            bestThreshold = 0;
            ...
            foreach (var threshold in PossibleThreshold(curSet,splitFeature))
            {
                //枚举所有可能的阈值
                double tmpResult = 0;
                double[] cntLabel_less = new double[MaxLabel];//小于等于阈值的数据
                double[] cntLabel_more = new double[MaxLabel];//大于阈值的数据
                double[] cntLabel_empty = new double[MaxLabel];
                double n_less = 0;
                double n_more = 0;
                double n_empty = 0;
                foreach(var ex in curSet.Examples)
                {
                    if (ex.features[splitFeature]==-1)
                    {
                        //特征为空
                        cntLabel_empty[ex.label]++;
                        n_empty++;
                    }
                    else if (ex.features[splitFeature]<=threshold)
                    {
                        //小于等于阈值
                        cntLabel_less[ex.label] ++;
                        n_less++;
                    }
                    else
                    {
                        //大于阈值
                        cntLabel_more[ex.label] ++;
                        n_more++;
                    }
                }
                //小于等于阈值
                ...
                for (int i = 0; i < MaxLabel; i++)
                {
                    if (n_empty == 0) continue;
                    double p = (double)cntLabel_empty[i] / (double)n_empty;
                    if (p == 0) continue;
                    tmp += ((double)n_empty / (double)N) * (-1 * (p * Math.Log(p, 2)));
                }
                tmpResult += tmp;
        ...
        public void GenerateSplit(DecisionTreeNode cur, int defaultLabel)
        {
            ...
            int[] cntLabel = new int[MaxLabel];
            foreach (var d in curSet.Examples) cntLabel[d.label]++;
            int max = cntLabel[0];
            int newDefaultLabel = 0;
            ...
            else
            {
                ...
                if (curSet.FeatureType[bestFeature] == 0)
                {
                    ...
                    emptyNode.split = new Split(cur.split);
                    emptyNode.split.splitCriteria[bestFeature] = bestThreshold;
                    emptyNode.split.splitOption[bestFeature] = -2;

                    cur.children = new DecisionTreeNode[3];
                    cur.children[0] = lessNode;
                    cur.children[1] = moreNode;
                    cur.children[2] = emptyNode;

                    //继续扩展子节点
                    GenerateSplit(cur.children[0], newDefaultLabel);
                    GenerateSplit(cur.children[1], newDefaultLabel);
                    GenerateSplit(cur.children[2], newDefaultLabel);
        ...
        public double GetIGR(int splitFeatrure)
        {
           
            DataSet curSet = dataset;
            double E_raw = Entropy(curSet);
            double IGR = 0;
            if (curSet.FeatureType[splitFeatrure] == 1)
            {
                //离散特征
                double H;
                double E_split = SplitEntropy_Discrete(curSet, splitFeatrure, out H);
                IGR = (E_raw - E_split) / H;
            }
            else
            {
                ...
                double threshold;
                double H;
                double E_split = SplitEntropy_Numeric(curSet, splitFeatrure, out threshold, out H);
                IGR = (E_raw - E_split) / H;
            }
            return IGR;
        }
